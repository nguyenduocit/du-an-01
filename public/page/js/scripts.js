...
$(document).on('click', '.gallery .img img', function(){
	$(this).parents('.item-img').find('.product-image img').attr('src', $(this).attr('data-src'));
});
...
if($('#hrv-menu').length){
		    var previousScroll = 0,
		        headerOrgOffset = $('#hrv-menu').offset().top;
		    
		    $(window).scroll(function() {
		        var currentScroll = $(this).scrollTop();
		        if(currentScroll > headerOrgOffset) {
		        	$('#hrv-menu').addClass('keep-menu');
		        	if(!$('#hrv_mene_clone').length) $('#hrv-menu').after('<div id="hrv_mene_clone" style="height: ' + $('#hrv-menu').height() + 'px"></div>');
		            $('#hrv-menu').stop(true, true).addClass('keep-menu-show').fadeIn();
		        } else {
		        	$('#hrv-menu').removeClass('keep-menu');
					$('#hrv_mene_clone').remove();
					$('#hrv-menu').fadeIn(0);
		        }
